...
class Zefoy:
	...
	def __init__(self):
		...
		self.url = 'None'
		self.text = 'NVA TOOL'
		url1=input("[NVATOOL]</> \033[1;33mNhập link video:\033[1;36m ")
		self.url=url1
	...
	def get_captcha(self):
		...
		try:
			...
			print('[NVATOOL]</> Đang giải capcha..')
			return False
		except Exception as e:
			print(f"[NVATOOL]</> Không thể giải captcha: {e}")
			time.sleep(2)
			self.get_captcha()

	def send_captcha(self, new_session = False):
		if new_session: self.session = requests.Session(); os.remove('session'); time.sleep(2)
		if self.get_captcha(): print('[NVATOOL]</> Đang kêt nối đến session');return (True, 'The session already exists')
		...
		if 'Enter Video URL' in request.text: 
			print('[NVATOOL]</> Session đã được tạo')
			open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
			print(f"[NVATOOL]</> Giải capcha thành công: {captcha_solve}")
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[NVATOOL]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return
			elif 'service is currently not working' in self.video_info: return (True,'[NVATOOL]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
				...
				elif lamtilo >= 1000: print('[NVATOOL]</> Your IP was banned')
				# print(f'[NVATOOL]</> Thời gian chờ để tiếp tục tăng: {t} giây')
				# for lam in range(t,0,-1):
				# 	lam-=1
				# 	print(f"[NVATOOL]</> Vui lòng chờ: {lam}",end="\r")
				# 	time.sleep(1)
				_=time.time()
				while time.time()-2<_+lamtilo:
					t-=1
					print("[NVATOOL]</> Vui lòng chờ: {0} ".format(t), end="\r")
					time.sleep(1)
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: print('[NVATOOL]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return ""
		elif 'Too many requests. Please slow' in res: time.sleep(3)
		elif 'service is currently not working' in res: return ('[NVATOOL]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[NVATOOL]</> Link video không hợ lệ'); return False
	...
	def update_name(self):
		while True:
			try:
				...
				self.text = f"[NVATOOL]</> | Views: {video_info['viewCount']} "
			except: pass
			time.sleep(5)
trang = "\033[1;37m"
xanh_la = "\033[1;32m"
...
def logo():
    ...
    logo=f"""
            ...
            \033[1;36m          ███╗   ██╗██╗   ██╗ █████╗ 
                      ...
                      ╚═╝  ╚═══╝  ╚═══╝  ╚═╝  ╚═╝                 \n\n\033[1;90m═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═  ═
                         ╔════════════════════╗
                         ║ \033[1;37mTOOL BUFF VIEW TT \033[1;90m ║
                         ╚════════════════════╝ \n"""
...
#     print(f"{trang}[NVATOOL]</> Link key: {link1}")
     
# else:
#     link1=""
# nhap = input(f"{trang}[NVATOOL]</> Nhập key vừa lấy được: ")
# if nhap == key:
if 1:
    print(f"{xanh_la}[NVATOOL]</> Key đúng mời bạn vô tool")
    logo()
    Z = Zefoy()
    ...
    print(f"{do}[NVATOOL]</> Key sai vui lòng vượt lại")
    exit()

while True:
	try: 
		if 'Service is currently not working, try again later' in str(Z.use_service()): print(f'{do}[NVATOOL]</>Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.');time.sleep(5)
	except Exception as e:print(f'Đang gặp sự cố | thử lại sau 10 giây.|| {e}');time.sleep(10)
